...
export default function MotivationPage() {
  ...
  const handleFavorite = () => {
    ...
    if (!isFavorited) {
      ...
      if (quote) {
        ...
        favorites.push({ ...quote, savedAt: new Date().toISOString() });
  ...
  const encouragementCards = [
    ...
    },
    {
      ...
      iconBg: 'bg-stone-600 dark:bg-stone-500',
      cardBg: 'glass-stone',
    },
    {
      icon: LightBulbIcon,
    ...
    <>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <motion.div
          ...
          className="glass-slate rounded-2xl shadow-slate p-8 md:p-12 mb-8 relative border border-slate-200 dark:border-slate-700"
        >
          <motion.button
          ...
          >
            {isFavorited ? (
              <HeartSolidIcon className="w-6 h-6 text-slate-700 dark:text-slate-300" />
            ) : (
          ...
          {quote && (
            <div className="mb-8">
              <svg
                className="w-12 h-12 text-slate-200 dark:text-slate-700 mb-4"
                fill="currentColor"
                viewBox="0 0 24 24"
                aria-hidden="true"
              >
              ...
              <blockquote className="text-2xl md:text-3xl text-slate-900 dark:text-white leading-relaxed mb-6">
                {quote.q}
              </blockquote>
              <p className="text-lg font-semibold text-slate-700 dark:text-slate-300">— {quote.a}</p>
            </div>
          )}

          <motion.button
            onClick={fetchDailyMotivation}
            className="w-full bg-slate-800 dark:bg-slate-200 hover:bg-slate-900 dark:hover:bg-white text-white dark:text-slate-900 py-3 rounded-full hover:shadow-lg transition-all font-semibold flex items-center justify-center gap-2 group"
          ...
          {encouragementCards.map((card, index) => {
            ...
            return (
              <motion.div
                ...
                className={`${card.cardBg} rounded-xl p-6 shadow-lg hover:shadow-xl transition-all transform hover:scale-105 border border-slate-200 dark:border-slate-700`}
              >
                ...
                <p className="text-slate-700 dark:text-slate-300 leading-relaxed">{card.message}</p>
              </motion.div>
            );
          })}
        </div>
